...
- **🏗️ Architectural Reorganization:** Split monolithic structures into domain-specific modules. The monolithic `OperationComponents.js` has been fractured into smaller, domain-driven classes (e.g., `TextOperationComponent`, `ImageOperationComponent`) inside `/src/js/OperationComponents/`. Utility functions have been similarly categorized into semantic subdirectories (`/src/js/utils/canvas/`, `/src/js/utils/`, etc.).
