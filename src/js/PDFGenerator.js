...
class PDFGenerator {
  ...
  static async drawTextOnPage(pdfDoc, pdfPage, operation) {
    ...
    const fontToEmbedKey = fontMap[fontFamily] || PDFLib.StandardFonts.Helvetica;
    let embedFontPromise = pdfDoc.__fontCache.get(fontToEmbedKey);
    if (!embedFontPromise) {
      embedFontPromise = pdfDoc.embedFont(fontToEmbedKey);
      pdfDoc.__fontCache.set(fontToEmbedKey, embedFontPromise);
    }
